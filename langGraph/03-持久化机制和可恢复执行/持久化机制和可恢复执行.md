...
检查点回溯（`Time Travel`）是一种在计算图执行过程中，根据历史检查点信息，回退到之前的状态，重新执行部分或全部节点的能力。
...
- `Replay`：不修改历史状态，只是沿着原先的执行路径重新执行后续节点。
- `Fork`：修改历史状态，从该位置创建一条新的执行分支。
...
如果想要实现`Replay`
需要把历史检查点保存到数据库中，然后通过`checkpoint_id`来恢复。
...
- `as_node`：指定这次状态更新应当被视为哪个节点产生的输出。
  会按照该节点的写入逻辑，将 `values` 写入状态通道，并据此决定后续从哪些节点继续执行。
